...
class FEKFSLAM:
    ...
    def publishOdometry(self, xk, Pk):
        ...
        print("FEKFSLAM odometry published")

    def main(self):
        # Main node execution
        rate = rospy.Rate(100)   # 100 Hz -> same as /odom
        while not rospy.is_shutdown():
            try:
                ...
                if self.imu_data is not None:
                    zk = np.copy(self.imu_data)
                    self.imu_data = None
                    print("zk:",zk,"self.imu_data:",self.imu_data)
                    # Update with IMU
                    print("IMU received")
                    self.xk, self.Pk = self.Update(zk, self.yaw_Rm, xk_bar, Pk_bar, self.Hm, self.Vm)
                else:
                    self.xk = xk_bar
                    self.Pk = Pk_bar
                    
                # except queue.Empty:     # no IMU message
                #     # print("IMU Queue empty")
                #     self.xk = xk_bar
                #     self.Pk = Pk_bar
                
                # Publish
                self.publishOdometry(self.xk, self.Pk)
