    ...
    * **The storage representation:** which is the representation used to store the feature in the map within the state vector.
    ...
    def Jhfx(self, xk):  # Jacobian wrt x of the feature observation function for all zf observations
        ...
        for i in range(len(self.zf)):
            Fj = self.Hp[i]
            if Fj is not None:
                _Jhf.append(self.Jhfjx(xk, Fj))

        if len(_Jhf) == 0:
            return np.zeros((0,0))
        else:
            return np.vstack(_Jhf)
    ...
    def Jhfv(self, xk):  # Jacobian wrt v of the observation function for a feature
        ...
        if self.zf is None:
            return np.zeros((0,0))
        for i in range(len(self.zf)):
            _Jv = scipy.linalg.block_diag(_Jv,np.eye(self.zfi_dim))
    ...
    def hfj(self, xk_bar, Fj):  # Observation function for zf_i and x_Fj
        ...
        # h(xk_bar,vk)=(-) xk_bar [+] x_Fj + vk -> BxF

        # TODO: To be implemented by the student
        NxB = self.GetRobotPose(xk_bar)
        NxF = self.M[Fj]
        _hfj = self.s2o(NxF.boxplus(NxB.ominus()))
        return _hfj
    ...
    def Jhfjx(self, xk, Fj):  # Jacobian wrt x of the observation function for feature observation i
        ...
        NxF = self.M[Fj]
        Jp = self.J_s2o(NxF.boxplus(NxB.ominus())) @ NxF.J_1boxplus(NxB.ominus()) @ NxB.J_ominus()  # position
        Jnp = 0     # not position
        J = [Jp,  Jnp]
        return Jp
    ...
    def Jgx(self, xk, BxFj):  # Jacobian wrt xk of the inverse sensor model for a single feature observation
        ...
        # BxFj = self.M[BxFj]
        NxB = xk
        Jp = self.o2s(BxFj).J_1boxplus(NxB) # position
        Jnp = 0     # not position
        J = [Jp, Jnp]
        return Jp
...
class Cartesian2DMapFeature(MapFeature):
    ...
    def GetFeatures(self):
        ...
        """
        # TODO: To be implemented by the student
        zk, Rk = self.robot.ReadCartesianFeatures() # [CartesianFeature1, CartesianFeature2...], cov[2x2]
